...
function toIssueFromLLM(iss, fallbackFile) {
    // iss may look like: { file?, title, severity, line?, rationale, suggestion? }
    const parts = [];
    if (iss?.title)
        parts.push(String(iss.title));
    if (iss?.rationale)
        parts.push(String(iss.rationale));
    if (iss?.suggestion)
        parts.push(`Suggestion: ${String(iss.suggestion)}`);
    return {
        file: iss?.file || fallbackFile,
        line: typeof iss?.line === "number" ? iss.line : undefined,
        message: parts.filter(Boolean).join(" — "),
        severity: mapSeverity((iss?.severity || "INFO")),
        rule: "llm",
    };
}
/** ---------------------------------------- */
(async () => {
    await consume(RABBITMQ_URL, QUEUE_NAME, async (msg) => {
        ...
        await updateFindings(payload.id, findings);
        console.log("reviewed", payload.id, payload.repo, `#${payload.pr_number}`, `${findings.issues.length} issues`, `(static:${findings.meta.static_issue_count || 0}, llm:${llmNewIssues.length})`);
    });
})();
